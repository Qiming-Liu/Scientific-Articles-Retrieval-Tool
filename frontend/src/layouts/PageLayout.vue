<template>
  <div class="page-layout">
    <page-header ref="pageHeader" :style="`margin-top: ${multiPage ? 0 : -24}px`" :title="pageTitle" :logo="logo"
                 :avatar="avatar">
...
export default {
  ...
  created() {
    this.page = this.$route.meta.page;
    this.$nextTick(() => {
      if (this.$router.currentRoute.path.startsWith('/Word')) {
        this.pageTitle = 'Keyword: [' + this.$route.query.name + ']';
      } else if (this.$router.currentRoute.path.startsWith('/Sentence')) {
        this.pageTitle = 'Question: [' + this.$route.query.text + ']';
      }
    });
  },
  beforeDestroy() {
